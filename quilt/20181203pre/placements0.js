...
function placements(hotrod, diskSizes) {
    ...
    this.three_per = function three_per() { 
        ...
        hotrod.redis.placeOn({diskSize: diskSizes[1]});
        hotrod.ingress.placeOn({diskSize: diskSizes[1]});
        hotrod.api_haproxy.placeOn({diskSize: diskSizes[1]});

        hotrod.api[1].placeOn({diskSize: diskSizes[2]});
        hotrod.map_haproxy.placeOn({diskSize: diskSizes[2]});
        ...
        hotrod.route[0].placeOn({diskSize: diskSizes[3]});
        hotrod.route[1].placeOn({diskSize: diskSizes[3]});

        hotrod.api[2].placeOn({diskSize: diskSizes[4]});
        hotrod.route[2].placeOn({diskSize: diskSizes[4]});
